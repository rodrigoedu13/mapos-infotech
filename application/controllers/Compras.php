...
class Compras extends CI_Controller {
    ...
    function adicionar() {
        ...
        if ($this->form_validation->run('compras') == false) {
            ...
            $data = array(
                ...
                'dataEmissao' => date("Y-m-d", strtotime(str_replace('/', '-', $this->input->post('dtEmissao')))),
                'situacao' => 0,
                'nrNfe' => $this->input->post('nfe')
            );

            if (is_numeric($id = $this->compras_model->add('compras', $data, true))) {
                $this->session->set_flashdata('success', 'Compra cadastrada com sucesso, adicione os produtos!');
                redirect(base_url() . 'compras/editar/'.$id);
            } else {
    ...
    function editar() {
        ...
        if ($this->form_validation->run('compras') == false) {
            ...
            if ($this->compras_model->edit('compras', $data, 'idCompra', $this->input->post('idCompra')) == TRUE) {
