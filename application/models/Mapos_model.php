...
class Mapos_model extends CI_Model {
    ...
    function getProdutosMinimo() {
        ...
        $sql = "SELECT * FROM produtos WHERE estoque <= estoqueMinimo";
        return $this->db->query($sql)->result();
    }

    function getProdutosComEstoque() {

        $sql = "SELECT * FROM produtos WHERE estoque >= 1";
        return $this->db->query($sql)->result();
    }
    ...
    public function getEstatisticasVendasMes() {
        $sql = "SELECT 
...
SUM(CASE WHEN (EXTRACT(MONTH FROM data_pagamento) = 12) AND baixado = 1 AND tipo = 'despesa' THEN valor END) AS VALOR_DEZ_DES
                FROM lancamentos  
                WHERE EXTRACT(YEAR FROM data_pagamento) = EXTRACT(YEAR FROM SYSDATE())";
        return $this->db->query($sql)->row();
    }
